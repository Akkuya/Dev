...
void wordSwapper(char *dest, char *src)
{
    ...
    // Move 'end' pointer to the end of the source string
    while (*end != '\0')
    {
        end++;
    }
    ...
    {
        word_end = end;
        while (word_end >= src && *word_end != ' ')
        {               // If current character is not a space, and still aftre the first charactger
            word_end--; // Move backwawrds by one
        }

        char *word_start = word_end + 1;
        while (word_start <= end)
        {                            // If the current_character is before end of the word
            *dest_ptr = *word_start; // Overwrite current character in destination with current character in source
            dest_ptr++;              // Increment destination and word_start pointers
            word_start++;
        }

        if (word_end >= src)
        {                    // If there is more characters to read in src
            *dest_ptr = ' '; // Add a space
            dest_ptr++;      // Increment destination pointer
        }

        // Move 'end' pointer to the character before the current word
        end = word_end - 1;
    }
    *dest_ptr = '\0'; // End the string with null terminator
}
#ifndef __TESTING
int main()
{
    char source[MAX_STR_LEN] = "silence .is a looking bird:the turning; edge, of life. e. e. cummings";
    char destination[MAX_STR_LEN] = "I am a destination string and I contain lots ofjunk 1234517265716572 @qsajdkuhasdgsahiehwjauhiuiuhdsj !";
    printf("The original string is: \n%s\n", source);

    wordSwapper(&destination[0], &source[0]);
    // Call word swapper with the address of the first character in 'source' and the first character in 'destination' wordSwapper(&destination[0], &source[0]);
    // You could also call wordSwapper like this: wordSwapper(destination,source)
    // since, as we will have seen in
    // lecture this week, the array's name can be used to pass into a function the
