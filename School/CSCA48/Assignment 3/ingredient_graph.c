...
intNode *related_with_restrictions(char source_ingredient[MAX_STR_LEN], char avoid[MAX_STR_LEN], int k_source, int k_avoid)
{
  ...
  intNode *current = source_head;
  while (current != NULL)
  {
    if (!searchInt(avoid_head, current->x))
    {
      result_head = insertInt(result_head, current->x);
    }
    current = current->next;
  }
  deleteList(source_head);
...
void substitute_ingredient(char recipe[10][MAX_STR_LEN], char to_change[MAX_STR_LEN])
{
  ...
  for (int i = 0; i < MAT_SIZE; i++)
  {
    ...
    for (int j = 0; j < 10; j++)
    {
      if (strcmp(recipe[j], ingredients[i]) == 0)
      {
        in_recipe = 1;
      }
    }
