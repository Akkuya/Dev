...
BST_Node *BST_delete(BST_Node *root, int bar, double index)
{
    ...
    else
    {
        ...
        {
            new_node = find_successor(root->right);
            root->bar = new_node->bar;
            root->freq = new_node->freq;
            root->index = new_node->index;
            root->key = new_node->key;
            root->right = BST_delete(root->right, new_node->bar, new_node->index);
            new_node = root;
        }
    }
...
BST_Node *findminBar(BST_Node *root, int bar) {
