...
def can_select(player: str,
               num_forwards: int,
               num_defencemen: int,
               num_goalies: int) -> bool:
    """Return True if and only if the number of players with the same
    position of player is less than the needed amount of players with the
    same position.
    If the player is a forward and num_forwards < FORWARDS_NEEDED, return True.
    If the player is a defencemen and num_defencemen < DEFENCEMEN_NEEDED, 
    return True.
    If the player is a goalie and num_goalies < GOALIES_NEEDED, return True.
    If player is the empty string, return True.

    Precondition: player is the string of player stats
                  as seen in players.txt,


    >>> can_select('GGG_PD_G0-_A14_DC43_H70_Pr5-', 0, 0, 0)
    True
    >>> can_select('GGG_PD_G0-_A14_DC43_H70_Pr5-', 5, 5, 5)
    False
    >>> can_select('', 0, 0, 0)
    True
    ...
    if position == FORWARD and (num_forwards < FORWARDS_NEEDED):
        return True
    if position == DEFENCEMEN and (num_defencemen < DEFENCEMEN_NEEDED):
        return True
    if position == GOALIE and (num_goalies < GOALIES_NEEDED):
        return True
...
def update_budget(budget: int, player: str) -> int:
    """Return the updated budget after subtracting the price of
    player from budget. If player string is empty, return
...
def remove_player(player_ids: str, player_index: int) -> str:
    ...
    Precondition: players_available is the string of player ids
                  that are currently drafted for selection seperated
                  by _.
...
def compute_goal_points(player: str) -> int:
    """
    Return the goal points of player. If the player is a goalie
    or an empty string, return 0.
    ...
    """
    if get_position(player) == GOALIE or len(player) == 0:
        return 0
    goals = player[8:10]
    if goals[1] == '-':
        goals = goals[0]
...
def compute_assist_points(player: str) -> int:
    """Return the assist points of player. If the player is a goalie
    or an empty string, return 0.
...
def compute_hit_points(player: str) -> float:
    """Return the hit points of player. If the player is a goalie
    or an empty string, return 0.
...
def compute_fantasy_score(player: str) -> float:
    """Return the fantasy score of player. If player is an
    ...
    if get_position(player) == GOALIE:
        gaa = player[10:14]
        sv = player[17:22]
        return (float(sv) * SV_VALUE) - (float(gaa) * GAA_VALUE)

    score = compute_assist_points(player)
    score += compute_goal_points(player)
    score += compute_dc_points(player)
    score += compute_hit_points(player)
    return score
